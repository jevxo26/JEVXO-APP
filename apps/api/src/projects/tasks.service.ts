...
export class TasksService {
  ...
  async remove(id: number) {
    ...
    return { message: `Task with ID ${id} has been moved to trash` };
  }

  // Get all trashed tasks for a project
  async findAllTrashed(projectId: number) {
    const tasks = await this.taskRepository.find({
      where: {
        projectId,
  ...
  async restore(id: number) {
    ...
    const restoredTask = await this.taskRepository.recover(task);
    
    // Update project task counts
    await this.updateProjectTaskCounts(task.projectId);
    
    return this.formatTaskResponse(restoredTask);
  }
  ...
  async permanentRemove(id: number) {
    ...
    // Update project task counts if task was not already deleted
    if (!task.deletedAt) {
      await this.updateProjectTaskCounts(projectId);
    }
    
    return { message: `Task with ID ${id} has been permanently deleted` };
  }

  // Cron job to auto-delete tasks older than 14 days
  @Cron(CronExpression.EVERY_DAY_AT_MIDNIGHT)
  async cleanUpTrash() {
    const fourteenDaysAgo = new Date();
    fourteenDaysAgo.setDate(fourteenDaysAgo.getDate() - 14);
    
    const oldTrashedTasks = await this.taskRepository.find({
      where: {
    ...
    if (oldTrashedTasks.length > 0) {
      await this.taskRepository.remove(oldTrashedTasks);
      this.logger.log(`Cleaned up ${oldTrashedTasks.length} old trashed tasks`);
    }
  }
